...
void loop() {
  ...
  if (ext_left == 0 && ext_right == 0 && (line_keepers_detects_black[0] == 1 || line_keepers_detects_black[1] == 1 || line_keepers_detects_black[2] == 1 || line_keepers_detects_black[3] == 1 || line_keepers_detects_black[4] == 1)) {
    ...
    // delay(50);
    // tempTstoreL = ext_left;
    // tempTstoreR = ext_right;
    // delay(60);
    // update_line_keepers();
    // extreme_ir();
    // tempTstoreL = tempTstoreL == 0 ? ext_left:tempTstoreL;
    // tempTstoreR = tempTstoreR == 0 ? ext_right:tempTstoreR;

    if (T_point_found()) {
      // if (tempTstoreL && tempTstoreR) {
      // _straight();/
      delay(25);
      front_ir();
